...
void parseCmdLine(int arg_num, char *arg_vet[])
{
	...
	for (int i = 1; i < arg_num; i++) {
		...
		//lcz modify 2024/1/8
		else if (!strcmp(arg_vet[i], "-approx_compute")){				
		...
		else if (!strcmp(arg_vet[i], "-approx_threshold"))				
		...
		else if (!strcmp(arg_vet[i], "-is_drop_trunc"))				
		...
		else if (!strcmp(arg_vet[i], "-NNapprox")){
			if(NoximGlobalParams::tdm == 1)
		...
		//MODIFY BY LCZ
		else if (!strcmp(arg_vet[i], "-NNapprox_Level_Table")){
			if(NoximGlobalParams::tdm == 1)
		...
		else if (!strcmp(arg_vet[i], "-NNlabel")){
		...
		//**********************************************************//
		else if(!strcmp(arg_vet[i], "-isapprox"))
			NoximGlobalParams::approx = atoi(arg_vet[++i]);
		//lcz modify 2024.1.30
		else if(!strcmp(arg_vet[i], "-allzeropacket"))
			NoximGlobalParams::allzero_packet = atoi(arg_vet[++i]);	
		else if(!strcmp(arg_vet[i], "-acdc_abdtr"))
