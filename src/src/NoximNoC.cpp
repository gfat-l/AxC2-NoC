...
void NoximNoC::buildMesh()
{
	...
	for ( i = 0; i < NoximGlobalParams::mesh_dim_x; i++) {
		...
		//siyue modify
		t[i][j][k]->pe->configure(k * NoximGlobalParams::mesh_dim_x*NoximGlobalParams::mesh_dim_y + j*NoximGlobalParams::mesh_dim_x +i,
				  NoximGlobalParams::stats_warm_up_time);
		t[i][j][k]->r->NN_Model = t[i][j][k]->pe->NN_Model;
...
//			ack_to_north[i][0][k] = bfs;
			req_to_north[i][NoximGlobalParams::mesh_dim_y][k] = 0;
//			ack_to_south[i][NoximGlobalParams::mesh_dim_y][k]= bfs;
	...
	for( j=0; j<=NoximGlobalParams::mesh_dim_y; j++){
		for( k=0; k<=NoximGlobalParams::mesh_dim_z; k++){
			req_to_east[0][j][k] = 0;
//			ack_to_west[0][j][k] = bfs;
			req_to_west[NoximGlobalParams::mesh_dim_x][j][k] = 0;
//			ack_to_east[NoximGlobalParams::mesh_dim_x][j][k]= bfs;
...
//			ack_to_down[i][j][NoximGlobalParams::mesh_dim_z] = bfs;
