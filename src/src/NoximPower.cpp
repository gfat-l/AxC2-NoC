...
// ---------------------------------------------------------------------------
//change lsy
double NoximPower::getRouterPower()
{
  return pwr_nn_router;
}

//change lsy
double NoximPower::getMEMPower()
{
  return pwr_nn_mem;
}   

//change lsy
double NoximPower::getComputePower()
{
  return pwr_nn_pe;
}
	
void NoximPower::compute(int mul) //lcz modify
{
  if(mul == 0) //精确
    pwr_nn_pe += ENERGY_MAC;
  else if(mul == 1) //移位
      pwr_nn_pe += ENERGY_MAC;
  else if(mul == 3) //有偏乘法器
      pwr_nn_pe += ENERGY_BIASED;
  else if(mul == 4) //有偏乘法器
      pwr_nn_pe += ENERGY_UNBIASED;
  else if(mul == 5 ) //有偏乘法器
    pwr_nn_pe += ENERGY_SHIFT;
  else if(mul == 6 ) //精确
    pwr_nn_pe += ENERGY_MAC;
}
//end modify

void NoximPower::memory_all()
{
  pwr_nn_mem += ENERGY_MEM_ALL;
}

void NoximPower::memory_single()
{
  pwr_nn_mem += ENERGY_MEM;
}

void NoximPower::router()
{
  pwr_nn_router += ENERGY_ROUTER;
}
