...
//	cout<<getCurrentCycleNum()<<"--"<<"flit.src_id: "<<flit.src_id<<"--"<<"flit.dst_id: "<<flit.dst_id<<"--"<<i<<endl;
...
unsigned int NoximStats::getReceivedPackets()
{
	...
	//cout<<"i: "<<i<<" packets: "<<chist[i].delays.size()<<endl;
	}
    return n;
}
...
unsigned int NoximStats::getReceivedFlits()
{
	...
	//cout<<"i: "<<i<<"--"<<chist[i].total_received_flits<<endl;
	}
    return n;
}
