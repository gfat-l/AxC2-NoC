...
SC_MODULE(NoximRouter)
{
    ...
    sc_out <TBufferFullStatus> ack_rx[DIRECTIONS + 1];	  // The outgoing ack signals associated with the input channels
    ...
    sc_in <TBufferFullStatus> ack_tx[DIRECTIONS + 1];	  // The outgoing ack signals associated with the output channels

    sc_out <int> free_slots[DIRECTIONS + 1];
    sc_in <int> free_slots_neighbor[DIRECTIONS + 1];
	/*******THROTTLING******/
	sc_out<bool>	 on_off[4];				  //�i���F��router�ۤv�O�_���Qthrottle neighbor
	sc_in<bool>		 on_off_neighbor[4];	  //�P�_�F��router�O�_�Qthrottle neighbor, �ӨM�w�O�_¶�}
	/*******THROTTLING******/
	/*******DOWNWARD ROUTING******/
	sc_out<int>		 DW_tag[4];				  //�ھڦ�pillar��traffic���p�ҨM�w�X�i�H�ϥΪ�DW level
	sc_in<int>		 DW_tag_neighbor[4]; 	  //�F��router�i�H�ϥΪ�DW level
	/*******DOWNWARD ROUTING******/
    /*******RCA******/
    ...
    int selection_type;
	BufferBank buffer[DIRECTIONS + 1];	        // Buffer for each input channel 
	vector<vector<pair<int, int> > > record;
    vector<vector<int> > vc_list;
	int start_from_port[DIRECTIONS+1];	     // Port from which to start the reservation cycle
    int start_from_vc[DIRECTIONS+1]; // VC from which to start the reservation cycle for the specific port
    int m_port_requests[DIRECTIONS+1];
    int m_vc_winners[DIRECTIONS+1];
